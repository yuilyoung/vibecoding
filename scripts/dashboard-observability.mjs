...
const run = (root, script) => { const absolute = join(root, script); if (!existsSync(absolute)) return { ok: false, source: script, observedAt: null, error: "Collector script is missing." }; const result = spawnSync(process.execPath, [absolute], { cwd: root, encoding: "utf8", timeout: 8_000 }); if (result.error) return { ok: false, source: script, observedAt: new Date().toISOString(), error: result.error.message }; if (result.status !== 0) return { ok: false, source: script, observedAt: new Date().toISOString(), error: (result.stderr || result.stdout || "Collector failed.").trim() }; try { return { ok: true, source: script, observedAt: new Date().toISOString(), value: JSON.parse(result.stdout) }; } catch { return { ok: false, source: script, observedAt: new Date().toISOString(), error: "Collector did not return JSON." }; } };
