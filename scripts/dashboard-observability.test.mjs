...
None.
`;
const temporaryRoot = () => { const root = mkdtempSync(path.join(tmpdir(), "dashboard-v4-")); const reportPath = path.join(root, "workspace", "2D-FPS-game", "docs", "reports"); mkdirSync(reportPath, { recursive: true }); mkdirSync(path.join(root, "dashboard"), { recursive: true }); writeFileSync(path.join(reportPath, "project-status.md"), report); writeFileSync(path.join(root, "dashboard", "index.html"), "<!doctype html><title>Control Plane</title>"); return root; };
...
test("v5 combines durable WBS tasks and execution quality without runtime demotion", () => {
  ...
  const planning = path.join(root, "workspace", "2D-FPS-game", "docs", "planning"), handoffs = path.join(root, "docs", "handoffs");
...
test("portfolio discovers work projects deterministically and preserves unknown versus blocked semantics", () => {
  const root = temporaryRoot(), arsRoot = path.join(root, "workspace", "animation_real_studio"), genericRoot = path.join(root, "workspace", "zeta-tool"), unicodeRoot = path.join(root, "workspace", "한글도구"), collisionA = path.join(root, "workspace", "a b"), collisionB = path.join(root, "workspace", "a+b"), reservedA=path.join(root,"workspace","unassigned"),reservedB=path.join(root,"workspace","workspace");
  ...
  writeFileSync(path.join(root, "workspace", "README.txt"), "not a project directory");
  ...
  assert.deepEqual(projects.map((project)=>project.path), ["workspace/2D-FPS-game", "workspace/a b", "workspace/a+b", "workspace/animation_real_studio", "workspace/unassigned", "workspace/workspace", "workspace/zeta-tool", "workspace/한글도구"]);assert.equal(new Set(projects.map((project)=>project.projectId)).size,projects.length);
  ...
  const ars=projects.find((project)=>project.projectId==="animation_real_studio"),generic=projects.find((project)=>project.path==="workspace/zeta-tool"),unicode=projects.find((project)=>project.path==="workspace/한글도구"),collisionIds=projects.filter((project)=>["workspace/a b","workspace/a+b"].includes(project.path)).map((project)=>project.projectId),reservedIds=projects.filter((project)=>["workspace/unassigned","workspace/workspace"].includes(project.path)).map((project)=>project.projectId);assert.notEqual(collisionIds[0],collisionIds[1]);assert.equal(reservedIds.includes("unassigned"),false);assert.equal(reservedIds.includes("workspace"),false);assert.equal(ars.progress.done,0);assert.equal(ars.progress.total,2);assert.equal(ars.completeness.percentage,0);assert.equal(ars.kanban.columns.decision[0].taskId,"ARS-001");assert.equal(ars.kanban.columns.blocked[0].taskId,"ARS-002");assert.equal(generic.progress.status,"unknown");assert.equal(generic.completeness.status,"unknown");assert.equal(unicode.displayName,"한글도구");assert.match(unicode.projectId,/^[a-z0-9][a-z0-9._-]{0,95}$/i);
  rmSync(root, { recursive: true, force: true });
});
test("2D adapter trusts incomplete tasks over a contradictory complete plan label", () => {
  const root=temporaryRoot(),reportPath=path.join(root,"workspace","2D-FPS-game","docs","reports","project-status.md"),planning=path.join(root,"workspace","2D-FPS-game","docs","planning");mkdirSync(planning,{recursive:true});writeFileSync(reportPath,report.replaceAll("Phase 8 - Environment Audio Polish","Phase 9 - Dashboard Operations").replace("(complete)","(active)"));writeFileSync(path.join(planning,"phase9-tasks.json"),JSON.stringify({phase:"phase-9",title:"Dashboard Operations",status:"complete",tasks:[{id:"T0",subject:"Plan",status:"completed"},{id:"T1",subject:"Review",status:"in-progress"}]}));
...
test("project-scoped events and usage never contaminate another portfolio project", () => {
  const root=temporaryRoot(),eventPath=defaultEventPath(root),now=new Date("2026-08-04T12:00:00Z"),arsRoot=path.join(root,"workspace","animation_real_studio"),reservedProjectId=deriveDashboardProjectId("unassigned");mkdirSync(path.join(arsRoot,"tasks"),{recursive:true});mkdirSync(path.join(root,"workspace","unassigned"),{recursive:true});writeFileSync(path.join(arsRoot,"tasks","mvp-readiness.json"),JSON.stringify({milestones:[{id:"M0",name:"Ready"}],tasks:[{id:"ARS-001",title:"Decision",status:"ready",milestone:"M0"}]}));
