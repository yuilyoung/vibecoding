...
const activeProject = { id: "photo-0001", status: "in_progress", mode: "text_to_photo", conditions, detailPrompt: "An original adult-safe rainy city scene with natural cinematic light.", composedPrompt: "server composed", output: { kind: "still", frameCount: 1, fps: null }, source: null, job: { id: "photo-job-photo-0001", provider: "codex-headless-imagegen", mode: "text_to_photo", outputKind: "still", requestedFrameCount: 1, encodedFrameCount: 0, fps: null, status: "in_progress", phase: "provider_started", progress: 55, progressBasis: "observed_server_lifecycle", startedAt: new Date(Date.now() - 20_000).toISOString(), phaseStartedAt: new Date(Date.now() - 20_000).toISOString(), completedAt: null, estimatedDurationSeconds: null, elapsedSeconds: 20, estimatedRemainingSeconds: null, etaState: "awaiting_observed_samples", durationSampleCount: 0 }, delivery: null, error: null };
const completedProject = { ...activeProject, status: "completed", job: { ...activeProject.job, status: "completed", phase: "completed", progress: 100, completedAt: new Date().toISOString(), estimatedRemainingSeconds: 0, etaState: "terminal" }, delivery: { mode: "text_to_photo", output: { kind: "still", frameCount: 1, fps: null }, notice: "done", asset: { id: "result", kind: "user_photorealistic_still", origin: "codex_headless_imagegen", generatedByAi: true, mimeType: "image/png", width: 9, height: 16, aspectRatio: "9:16", frameCount: 1, fps: null, durationSeconds: null, byteLength: 2, dataUri: "data:image/png;base64,AA==", notice: "local result", cleanupWarning: { code: "cleanup_warning", osCode: "EBUSY" } } } };
const forecastProject = { ...activeProject, job: { ...activeProject.job, progress: 72, observedProgress: 55, forecastHighWater: 72, progressBasis: "server_lifecycle_and_duration_forecast", estimatedDurationSeconds: 75, elapsedSeconds: 60, estimatedRemainingSeconds: 15, etaState: "estimated", durationSampleCount: 3 } };
...
test("real photo studio displays server-calculated duration forecast progress and only displays 100 after a completed result", async ({ page }) => {
  ...
  const bar = job.getByRole("progressbar", { name: "관측된 서버 생명주기 진행률" });
  ...
  await expect(job).toContainText("모델 렌더 비율이 아닌 서버 확인 단계입니다.");
  await expect(job).toContainText("\uB3D9\uC77C \uC720\uD615 \uC644\uB8CC \uD45C\uBCF8 3\uAC1C\uB97C \uBAA8\uC73C\uB294 \uC911");
  const forecastBar = job.getByRole("progressbar", { name: "\uC11C\uBC84 \uB2E8\uACC4 \uBC0F \uC644\uB8CC \uC2DC\uAC04 \uD45C\uBCF8 \uAE30\uBC18 \uC608\uC0C1 \uC9C4\uD589\uB960" });
  await expect(forecastBar).toHaveAttribute("aria-valuenow", "72");
  await expect(job).toContainText("\uC11C\uBC84 \uC0DD\uBA85\uC8FC\uAE30\uC640 \uB3D9\uC77C \uC720\uD615\uC758 \uC2E4\uC81C \uC644\uB8CC \uC2DC\uAC04 \uD45C\uBCF8\uC73C\uB85C \uACC4\uC0B0\uD55C \uC644\uB8CC \uC608\uC0C1\uCE58\uC785\uB2C8\uB2E4.");
...
test("real photo studio submits 2 and 30 frame GIF boundaries and renders observed GIF encoding", async ({ page }) => {
  const gifActive = { ...activeProject, output: { kind: "motion_gif", frameCount: 30, fps: 10 }, job: { ...activeProject.job, outputKind: "motion_gif", requestedFrameCount: 30, encodedFrameCount: 15, fps: 10, phase: "gif_encoding", progress: 95 } };
  const gifCompleted = { ...gifActive, status: "completed", job: { ...gifActive.job, status: "completed", phase: "completed", progress: 100, encodedFrameCount: 30, completedAt: new Date().toISOString(), estimatedRemainingSeconds: 0, etaState: "terminal" }, delivery: { mode: "text_to_photo", output: { kind: "motion_gif", frameCount: 30, fps: 10 }, notice: "motion done", asset: { id: "gif", kind: "user_motion_gif", origin: "local_motion_gif_from_generated_still", generatedByAi: true, mimeType: "image/gif", width: 432, height: 768, aspectRatio: "9:16", frameCount: 30, fps: 10, durationSeconds: 3, byteLength: 1024, dataUri: "data:image/gif;base64,R0lGODlh", notice: "motion gif" } } };
  let reads = 0;
  await page.route("**/api/photorealistic-projects", async (route) => {
    const request = route.request().postDataJSON();
...
test("real photo studio keeps an exceeded estimate below 100 and renders an asynchronous provider failure", async ({ page }) => {
  ...
  const failedProject = { ...overdueProject, status: "failed", job: { ...overdueProject.job, status: "failed", phase: "failed", progress: 90, completedAt: new Date().toISOString(), estimatedRemainingSeconds: 0, etaState: "terminal" }, error: { code: "provider_failed", message: "The local provider stopped after starting." } };
