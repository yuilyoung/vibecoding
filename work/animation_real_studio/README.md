...
`STUDIO_HEADLESS_IMAGEGEN=1` is an opt-in local-development switch. With it enabled, open `/real` to generate one actual 9:16 photorealistic still from a structured prompt or from one user-attested original 2D PNG/JPEG/WebP still; in 2D-to-photo mode only, that source is staged in a disposable workspace and attached to the signed-in Codex request as visual input. The UI always shows observed server lifecycle and GIF-frame progress. Before three completed jobs exist in the same mode/output bucket of the current local API session, the server supplies a clearly labelled bootstrap estimate from the configured provider deadline plus a bounded output-validation/encoding buffer; its timer starts only after the actual provider-started milestone and advances elapsed-time forecast in 5% increments only up to 90%. At three samples, the source automatically becomes the bucket median without lowering the already visible high-water progress. At 90%, the interface changes to a completion-finalization panel: image validation, GIF-frame encoding, artifact saving, or a pending provider completion event can be shown there while the numeric bar remains at 90%. Only a validated terminal artifact reaches 100%; failed jobs never do. A selected 2 to 30 frame/10fps motion GIF is a deterministic local pan/zoom derived from that one generated still, not AI video or per-frame image generation. This path is not a public or production media service. See [the experiment boundary](docs/11-trusted-local-photorealistic-experiment.md).
