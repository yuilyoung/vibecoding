...
| [eference-media-intake](../.codex/skills/reference-media-intake/SKILL.md) | 참조 업로드·공급자 전달 전 | 이미지/짧은 영상 → 격리·정제·판정·삭제·사용 이력 | 인물·원작 장면 재현, 원본 공개 |
...
```text
adaptation-intake → reference-media-intake → rights-safety-gate → vertical-short-director
                   ↓ (검토/차단)                                      ↓ (사용자 승인)
...
- 여섯 스킬은 구현 코드가 생긴 뒤 단위 테스트/통합 테스트의 입력 fixture와 운영 문구의 기준으로도 사용한다.
