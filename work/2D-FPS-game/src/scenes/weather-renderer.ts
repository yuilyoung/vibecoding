...
export class WeatherRenderer {
  ...
  private syncParticlePool(): void {
    const targetCount = this.getTargetParticleCount();

    while (this.particles.length < targetCount) {
      const sprite = this.scene.add.rectangle(0, 0, this.currentWeather.type === "rain" ? 2 : 6, this.currentWeather.type === "rain" ? 16 : 2, 0xffffff, 0.42)
        .setDepth(39)
        .setScrollFactor(0);
      const particle = { sprite, velocityX: 0, velocityY: 0 };
    ...
    for (const particle of this.particles) {
      particle.sprite
        .setSize(this.currentWeather.type === "rain" ? 2 : 6, this.currentWeather.type === "rain" ? 16 : 2)
        .setFillStyle(this.currentWeather.type === "sandstorm" ? 0xe9c46a : 0xbde0fe, this.currentWeather.type === "sandstorm" ? 0.38 : 0.42);
      this.resetParticle(particle, VIEW_WIDTH, VIEW_HEIGHT, true);
    }
  }
  ...
  private getTargetParticleCount(): number {
    if (this.currentWeather.type === "rain" || this.currentWeather.type === "sandstorm") {
      return Math.max(0, this.weatherConfig.types[this.currentWeather.type].particleCount);
    }
