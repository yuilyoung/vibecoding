...
`npm run assets:world-object-atlas` verifies the twenty immutable alpha PNG sources and approved v5 target-frame hash, performs two isolated Sharp 0.35.3 builds, compares every output byte, validates the 2048x1024 lossless WebP/Phaser JSON/schema 1.1.0 manifest and budgets, then atomically promotes the complete release.
