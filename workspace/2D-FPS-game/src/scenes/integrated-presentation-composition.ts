...
import { PresentationEventPolicy, type PresentationEvent, type PresentationEventPort } from "../domain/visual/PresentationEvent";
...
export class IntegratedPresentationComposition implements PresentationEventPort {
  ...
  public publish(event: PresentationEvent): void {
    ...
    const color = instruction.kind === "destroy" ? 0xff8a3d : 0xffe08a;
    const radius = instruction.kind === "destroy" ? 22 : 15;
    const effect = this.scene.add.circle(instruction.x, instruction.y, radius, color, 0.62).setDepth(8);
    this.effects.add(effect);
    this.scene.tweens.add({
      targets: effect,
      alpha: 0,
      scale: instruction.kind === "destroy" ? 1.8 : 1.35,
      duration: instruction.kind === "destroy" ? 210 : 130,
