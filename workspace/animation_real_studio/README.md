...
`STUDIO_HEADLESS_IMAGEGEN=1` is an opt-in local-development switch. With it enabled, open `/real` to generate one actual 9:16 photorealistic still from a structured prompt or from one user-attested original 2D PNG/JPEG/WebP still; in 2D-to-photo mode only, that source is staged in a disposable workspace and attached to the signed-in Codex request as visual input. Start the API from a trusted local terminal that is allowed to launch child processes. After the Codex provider actually starts, the numeric forecast begins at 5%; the server then divides its current completion estimate into 100 equal time slices and adds 1% per completed slice, monotonically up to a 90% cap. At three completed samples in the same mode/output bucket, the estimate source switches from the bootstrap deadline to the bucket median without lowering visible progress. At 90%, the interface shows an explicit completion-wait panel for provider output, image validation, GIF encoding, or artifact saving while the numeric bar remains at 90%. Only a validated terminal artifact reaches 100%; failed jobs never do. A selected 2 to 30 frame/10fps motion GIF is a deterministic local pan/zoom derived from that one generated still, not AI video or per-frame image generation. This path is not a public or production media service. See [the experiment boundary](docs/11-trusted-local-photorealistic-experiment.md).
