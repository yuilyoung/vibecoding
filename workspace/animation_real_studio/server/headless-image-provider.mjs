...
export class HeadlessCodexImageProvider {
  ...
  async generateUserImageAttempt({ prompt, reference = null, output = { kind: "still", frameCount: 1 }, onPhase = () => {} } = {}) {
    ...
    const sourceInstruction = reference ? "Reference handling: use the attached user-attested original 2D image only through the requested visual domains in the validated render contract. First compare the source with that contract, then preserve the selected non-conflicting visual anchors while letting the story direction control action, emotion, event, and intentional scene change. Never infer a real-person identity, copyrighted character, protected distinctive design, logo, signature, watermark, or readable text from the source; never copy pixels or an unchanged scene when the story requests a change." : "There is no source image. Create the image from the validated visual brief and story direction only.";
    const providerPrompt = `Use $imagegen to create exactly one original, non-identifying photorealistic still for the trusted-local Animation Real Studio experiment.\n\n${sourceInstruction}\n\n${prompt.trim()}\n\nFollow the validated story direction for action, emotion, event, and intentional scene change; use the source only for non-conflicting visual continuity.\nComposition/framing: vertical 9:16 portrait, 1152 by 2048 pixels.\nHard constraints: all people must be clearly fictional adults age 20+; non-graphic sensual styling is allowed only when consensual; do not create minors, age-ambiguous people, explicit sexual activity, pornography, coercive sexual scenarios, a recognisable real person, celebrity, copyrighted character, logo, signature, watermark, readable text, graphic violence, or gore.\nSave the final PNG to this exact relative path: result.png.\nDo not read, create, or edit any other file except the attached reference and result.png. In your final response, report only whether result.png was saved.`;
    let asset;
    let primaryError;
    try {
      ...
      const execution = await this.run({ command: this.command, args: ["exec", "--ephemeral", "--sandbox", "workspace-write", "--skip-git-repo-check", "--cd", workspace, ...(stagedReferencePath ? ["--image", stagedReferencePath] : []), providerPrompt], cwd: workspace, environment: sanitizedEnvironment(this.environment), timeoutMs: this.timeoutMs, outputPath: isolatedOutputPath, prompt: providerPrompt, onStarted: onProviderStarted });
      if (execution.exitCode !== 0) throw providerFailure(execution);
      let bytes;
